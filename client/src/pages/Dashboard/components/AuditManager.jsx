...
function describeDeleteResult(summary) {
  ...
  const parts = [
    ...
    `เปลี่ยนชำรุดกลับเป็นพร้อมใช้งาน ${summary.damaged_reverted ?? 0} ชิ้น`,
  ];
  // ใบแจ้งซ่อมมีเฉพาะรอบเก่าที่ผลชำรุดยังเปิดใบซ่อมให้อัตโนมัติ
  if (summary.repairs_cancelled > 0) {
    parts.push(`ยกเลิกใบแจ้งซ่อม ${summary.repairs_cancelled} ใบ`);
  }
  ...
  function renderDeleteConfirm(round) {
    ...
    return (
      <div className="audit-confirm">
        <p>
          {open
            ? `ยกเลิกรอบ "${round.title}"? ทุกอย่างที่รอบนี้เปลี่ยนไว้จะถูกย้อนคืน: ห้องที่ย้ายระหว่างตรวจจะย้ายกลับ และของที่รอบนี้เปลี่ยนเป็นชำรุดจะกลับเป็นพร้อมใช้งาน จากนั้นรอบนี้จะถูกลบถาวร`
            : `ลบรายงาน "${round.title}" ถาวร? ห้องและสถานะที่เปลี่ยนไประหว่างรอบนี้จะยังอยู่ตามเดิม ลบแล้วกู้คืนไม่ได้`}
        </p>
